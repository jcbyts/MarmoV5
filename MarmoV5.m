...
%     holdrop = 0;
%     dropreject = 0;
%     % track how long it takes to complete each step in the loop
%     LoopCounter = 1;
%     FrameStates = nan(25e3, 8);
%     StateNames = {'Start', 'GetState', 'GetEye', 'UpdateProtocol', 'Reward', 'Screen', 'GUI', 'continue'};
%     
%     screenTime = Screen('Flip', handles.A.window, 0); % get us started at the right time
%     
%     
    % ****************************
    handles = marmoview.runTrial(handles, hObject, PR);
%     %**************
%     while runloop
%        
...
%        state = PR.get_state();
% %        state = PR.state;
%        FrameStates(LoopCounter, 2) = GetSecs;
%        
%        %%%%% GET ON-LINE VALUES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%        [ex,ey] = handles.eyetrack.getgaze();
%        pupil = handles.eyetrack.getpupil();
...
%        %**********************************
% 
%        FrameStates(LoopCounter, 3) = GetSecs;
%        
%        
%        
%        
...
%        if (GetSecs - screenTime) > frameTime
%            
...
% %             [screenTime,~, ~, ~] = Screen('Flip',handles.A.window,0);
% %             if state~=1
% %                 updateGUI = true;
% %             else
% %                 updateGUI = false;
...
%     D.ENDCLOCK = ENDCLOCK;
%     D.FrameStates = FrameStates(1:LoopCounter,:);
%     D.StateNames = StateNames;
%     
