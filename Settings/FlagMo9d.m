...
P.dotdelay = 0.2; %0.2; %0.30 % extra duration stim presentation after saccade
S.dotdelay = 'Extra dot time(s):';
P.holdDur = 0.1;   % must hold or error 5
...
P.trialSwap = 20;  
S.trialSwap = 'Swap spatial config (flip up/down) every N correct:';
P.spatialconfig = 1;
S.spatailconfig = 'Arrangement of targets, (1 or 2):';
if (P.spatialconfig == 1)
   P.RF_X = -4.3; 
...
if (P.motionStimulus == 0)  % use grating      
    ...
    P.dotSpeed = 6.75; % 10 * (ecc/5); %15; 
    S.dotSpeed = 'Dot Speed (Degree per sec):';
    P.dotSize = 0.15; % * (ecc/5); 
    S.dotSize = 'Dot Size (Degree):';
    P.dotLife = Inf; %6; % or 6 frames
    S.dotLife = 'Dot lifetime:';
    P.dotBandwdthSteps = 1; %5; % number of steps in bandw size
    S.dotBandwdthSteps = 'number of bandw step (int,5):';
    P.dotBandwdthStepSize = 90; % size of step in bandw
    S.dotBandwdthStepSize = 'size of bandw step (90):';
    P.dotBandwdth = 0;  % 0 default or 180 (SS = 1 or 0.5)
    S.dotBandwdth = 'Coherence bandwidth (degs):';
    P.dotNum = 75; % floor( 50 * ((ecc/5)^ 2) );  %50
