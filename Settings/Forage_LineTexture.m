...
function [S,P] = Forage_LineTexture()
...
S.protocolTitle = 'Foraging with static line texture';
...
%         P.lightBarWidths = [1 2 1 4];
        ...
        P.orientation = [repmat(0, 1, numel(P.lightBarWidths)) ...
                repmat(90,1,numel(P.lightBarWidths))]; %#ok<*REPMAT>
            
        % start with light or dark?
        P.startPhase = [repmat(0, 1, numel(P.lightBarWidths)) ...
                repmat(0,1,numel(P.lightBarWidths))];
            
        % duplicate the line width conditions for the number of
        % orientations
        P.lightBarWidths = repmat(P.lightBarWidths, 1, 2);
        P.darkBarWidths = repmat(P.darkBarWidths, 1, 2);
        
        S.orientation = "cannot be changed from GUI";
        S.startPhase = "cannot be changed from GUI";
        
        P.linesContrast = 1.0;
        S.linesContrast = 'contrast of the lines (requires update):';
