...
P.CycleBackImage = 10;  % really pamper him this time
...
P.JumpHideOut = 0.66;  % probability to jump target to new location
S.JumpHideOut = 'Prob to jump to new target location: ';
%******
P.probNull = 0;
...
P.holdDur = P.dotdelay;   % must hold until feedback appers, or error 5
...
P.radius = 0.50 * ecc; %0.25 * ecc; % for MT is 0.5*ecc;  % diameter of target is dva
S.radius = 'Size of Target(dva):';
P.choiceRadius = 0.4 * ecc;
...
if (P.motionStimulus == 0)  % use grating      
    P.cpd = 2;  % must be scaled by pixperDeg
    ...
    P.driftSpeed = 5.0;
    S.driftSpeed = 'Speed of a drifting grading (deg per sec):';
else 
