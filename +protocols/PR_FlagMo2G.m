classdef PR_FlagMo2G < handle
  ...
  properties (Access = public), 
       ...
       HideOut double = 0;        % no specific location
       OverDraw double = [];      % history of saccades to impossible targets
  end
  ...
  methods (Access = public)
    ...
    function initFunc(o,S,P);
       ...
       o.error = 0;
       o.last_item = NaN;
       o.HideOut = 0;  % no history to start
       o.OverDraw = [20,20,20];  % start with equal distribution
       ...
       for k = 1:(2*o.targnum)
           if (P.motionStimulus == 1)
               ...
               o.hProbe{k} = stimuli.dgrating(o.winPtr);  % grating probe
               o.hProbe{k}.transparent = -P.probecon;  % blend in proportion to gauss
               o.hProbe{k}.gauss = true;
               o.hProbe{k}.square = (P.squareWave == 1);
               o.hProbe{k}.pixperdeg = S.pixPerDeg;
               o.hProbe{k}.driftSpeed = P.driftSpeed;
               o.hProbe{k}.framerate = S.frameRate;
               o.hProbe{k}.pattern = P.pattern;
           end
       end
    ...
    function P = next_trial(o,S,P);
            ...
            if (o.HideOut == 0)
               ...
               o.HideOut = o.target_item;  % init HideOut to target
            else
               if (o.last_item == o.HideOut)  % if choose hideout last time 
                  if (rand < o.P.JumpHideOut)
                     %****** should choose relative to past freq
                     drawtot = sum(o.OverDraw) * 10;
                     sampo = floor( drawtot ./ o.OverDraw );
                     sampo(o.HideOut) = 0;  % zero chance to pick same as before
                     ri = randi( sum(sampo) );
                     sumo = 0;
                     for ii = 1:length(o.OverDraw)
                         sumo = sumo + sampo(ii);
                         if (ri <= sumo)
                             it = ii;
                             break;
               ...
               o.target_item = o.HideOut;
               %***** no longer a constrain that o.last_item conflicts
               if (o.last_item == o.target_item)
                   o.last_item = NaN;
               end
               %********* update and over-ride target location 
               xps = P.RF_X; % P.xDeg;
               yps = P.RF_Y; % P.yDeg;
               ango = 2*pi*(o.target_item-1)/o.targnum;  % shift the position
               o.P.xDeg = (cos(ango) * xps) + (sin(ango) * yps);
               o.P.yDeg = (-sin(ango) * xps) + (cos(ango) * yps);
               P.xDeg = o.P.xDeg;
               P.yDeg = o.P.yDeg;
            ...
            for zk = 1:o.targnum
                ...
                if (o.P.motionStimulus)
                    ...
                    if isnan(P.postori)
                        o.hProbe{o.targnum+zk}.colour = repmat(P.bkgd,1,3);      % still have blanks (1/3 time) 
                    else
                       if (zk == o.target_null)
...
%                   o.hProbe{o.targnum+zk}.orientation = o.hProbe{zk}.orientation + o.deltaOri;  % into degrees/ direction of motion  
                end
            end
    end
    ...
    function drop = state_and_screen_update(o,currentTime,x,y) 
        ...
        if o.state == 5 && currentTime > o.responseStart + o.P.flightDur
            ...
            if (o.chooseTarget > 0)
                %***** keep stats on past choices, target face to equalize
                o.OverDraw(o.chooseTarget) = o.OverDraw(o.chooseTarget) + 1;  
                %*************
                ...
                if (o.P.fixation == 0) 
                    if (o.error == 5)
                       ...
                       if (currentTime > o.responseEnd + o.P.dotdelay)
                           ...
                           %****** showing post-saccadic targets (blank if
                           %****** P.postori == NaN ...
                           if (isnan(o.P.postori))
                             for k = 1:o.targnum
                      ...
                      if ~o.error  
                          %***** if correct fixation, show light Gauss
                          o.hReward{1}.position = o.Faces.position;
                          o.hReward{1}.beforeFrame();
                          %***** don't show face, distracting
                          % o.Faces.beforeFrame();  
                      end
                end
