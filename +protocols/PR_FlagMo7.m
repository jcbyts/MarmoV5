classdef PR_FlagMo7 < handle
  ...
  properties (Access = public), 
       ...
       fixExtra double = 0;       % extra fix time
  ...
  methods (Access = public)
    ...
    function P = next_trial(o,S,P);
            ...
            if P.runType == 1    
                     ...
                     %***************
                     P.xDeg = o.trialsList(i,1);   % position will be randomized
                     P.yDeg = o.trialsList(i,2);   % but kept inside the same hemifield
            ...
            %********** trials list will control + or - 90
            ...
            %******************
            %*** no cases with a null target anymore, instead location
            %*** unpredictable **********************************
            ...
            %********
            if ((P.RF_X * P.RF_Y) > 0)
                zango = -(pi/2);  % top right or bot left, alternate targ clockwise
            else
                zango = +(pi/2);  % top left or bot right, alternate targ counter-clockwise
            end
            %******          
            for zk = 1:o.targnum
                %***** rotate appropriate for target
                ...
                xp = (cos(ango) * xps) + (sin(ango) * yps);
                yp = (-sin(ango) * xps) + (cos(ango) * yps);
                %********* randomize targets via hemi-field here
                if (o.P.locNum == 2)  % if randomize loc per hemi-field
                  if (rand < (1.0 - o.P.probRF))  % either RF or opp RF, or 90 deg rotate
                      xpo = (cos(zango) * xp) - (sin(zango) * yp);
                      ypo = (sin(zango) * xp) + (cos(zango) * yp);
                      xp = xpo;
                      yp = ypo;
                  end   
                else  %full randomization
                  zango = -(pi/2) + (((randi(o.P.locNum)-0.5) / o.P.locNum) * pi);
                  nomo = norm([xp,yp]);
                  if (xp <= 0)
                    xp = -nomo*cos(zango);
                    yp = nomo*sin(zango);
                ...
                %***********************************
                if (zk == 1)
                    ori = P.ori;   % control ori for target, rand for other
                else
                    if (o.P.motionStimulus == 1)
                      ori = ((randi(o.P.orinum)-1) * 360 / o.P.orinum);  % random
                    else
                      ori = ((randi(o.P.orinum)-1) * 180 / o.P.orinum);  % random    
                    end
                end
            ...
            %**** reset point cue by randomized location
            %******* set cue point for target location
            o.hPoint.sigma1 = P.sigma1;
            xp = o.targ_x(o.target_item);
            yp = o.targ_y(o.target_item);
            o.hPoint.UpdateTextures(xp,yp);
            %**********
    ...
    function [FP,TS] = prep_run_trial(o)
            ...
            o.fixExtra = o.P.fixMinExtra + ceil(1000*o.P.fixRanExtra*rand)/1000;
             %******* jitter timing of Stim Onset
            o.stimOnset = o.P.cueOnDur + rand * (o.P.cueOnDel - o.P.cueOnDur);
            %*********************
            o.dotflip = 0;  % default
            o.flashCounter = 0;
            o.gotTarget = 0;
    ...
    function drop = state_and_screen_update(o,currentTime,x,y) 
        ...
        %***** go to error if she does not pick a target in first saccade
        if o.state == 5 && currentTime > o.responseStart + o.P.flightMax
            %******* determine if a stimulus is select by saccade
            if (o.chooseTargetFirst == 0) && (o.chooseTarget == 0)
                 o.state = 8; % Move to iti -- inter-trial interval
                 o.error = 7; % Error 2 is failure to hold fixation
                 o.itiStart = GetSecs;
            end
        end
        %**************
        ...
        if o.state == 7
            if ~o.error && o.rewardCount < o.P.rewardNumber
               if currentTime > o.itiStart + 0.2 * o.rewardCount  % deliver in 200 ms increments before face reward
                ...
                if currentTime > ( o.responseEnd + o.P.dotdelay2 + 0.1 )
                   o.state = 8;
                end
            end
        end
        ...
        switch o.state
            ...
            case 1
                ...
                % Hold fixation, grace period to obtain it (50ms)
                o.hFix.beforeFrame(1);  % very brief, in case were at
                                        % fixation as trial started
                
            case 2

                % continue to hold fixation for random delay (50-150)ms   
                o.hFix.beforeFrame(1);  % very brief, in case were at
                
            case 3   

                % continue to hold fixation during cue (50-200) ms   
                %***** cue for FixExtra period
                if (o.P.fixation == 0) && (o.P.cue_contrast)
                    if (o.P.showCue == 1)
                        if (currentTime < o.stimStart + o.P.cueOnDur)        
                ...
                %***** Total fixaton period, 150 to 400 ms ... variable
                %***** Cue onset varies at 100 to 200 ms in that span

                    
            case 4    % waiting for him to leave fixation 
                
                % continue to show fixation, disappear after FixExtra
                if (currentTime < o.stimTime + o.fixExtra)
                    o.hFix.beforeFrame(1);  % very brief, in case were at
                else
                    if (o.fixOffset == 0)
                        o.fixOffset = GetSecs;
    ...
    function plot_trace(o,handles)
        ...
        for zk = 1:o.targnum
           r = o.P.radius; 
           if (zk == o.target_item)
               plot(h,o.targ_x(zk)+rC*cos(0:.01:1*2*pi),o.targ_y(zk)+rC*sin(0:.01:1*2*pi),'r-');
           else
               if (zk == o.last_item)
                  plot(h,o.targ_x(zk)+rC*cos(0:.01:1*2*pi),o.targ_y(zk)+rC*sin(0:.01:1*2*pi),'b-');       
               else
                  plot(h,o.targ_x(zk)+rC*cos(0:.01:1*2*pi),o.targ_y(zk)+rC*sin(0:.01:1*2*pi),'k-');
               end
           end
        end
       %*********************
