classdef PR_Forage4 < handle
  ...
  properties (Access = private)
    ...
    FixJit = 0;        % jitter in time to reappear target after landing
  ...
  methods (Access = public)
    ...
    function initFunc(o,S,P);
       ...
       o.ProbeHistory = zeros(o.MaxFrame,5);  % x,y,ori,fixated,visible
       ...
       for kk = 1:o.oriNum
           ...
           if ( o.oriNum >= 8)
               if (kk < o.oriNum)
                  o.hProbe{kk}.orientation = (kk-1)*180/(o.oriNum-1);  %last one is blank
               else
    ...
    function reset_probe_location_and_texture(o,firstime,xx,yy)
        %******** store target info (will need memory for PR struct)
        oldPos = o.PosList;
        if (firstime)
          nego = (rand < 0.5);
          %***********  
          anga = (rand-0.5)* 2.0 * o.P.hortarget;
          if nego
              anga = anga + 180;
          end
          anga = anga * (pi / 180);
    ...
    %                    normo = 1;
    %                end
    %                oldPos = o.P.stimEcc * [(xx/normo),(yy/normo)];
    %             end
    %             %*******
    ...
    function P = next_trial(o,S,P);
          ...
          o.reset_probe_location_and_texture(1,NaN,NaN)
    end
    ...
    function [drop,faceitem] = updateProbes(o,xx,yy,currentTime)
        ...
        % check if fixating on the target
        itfix = 0;
        ...
        if (o.state == 1)  % if state == 2 or 3, then no stim visible to check
            %**********************************************************
            if (o.stimOn > 0)  % check if stim no acquired, if so re-flash
               if (currentTime > (o.stimOn+o.P.stimWait))
            ...
            if (itfix > 0)
                ...
                o.state = 2;  
                %******* any re-appear contingencies could be done here
                if (rand < o.P.probchange)
                    if o.oriNum > 2
                      nori = randi((o.oriNum-1));   % integer of ori, 1 to orinum
                    else
                      nori = randi(o.oriNum);
                    end
                    o.hProbe{nori}.position = o.hProbe{o.targOri}.position;
                    o.targOri = nori;
                end
                %************************************************
            end
        ...
        if (o.state == 2)
            if (itfix == 0)  % fixation was broken from target
                % if fixation not held, no reward and reset target
                o.reset_probe_location_and_texture(0,xx,yy); % make it jump, firstime=1    
                o.FixTime = 0;  % reset, they broke fixation
                o.stimOn = 0;               
                o.state = 1;
            else
                %*******
                if (currentTime > (o.FixTime + o.FixJit + o.P.holdDur))
                    if (o.targOri < o.oriNum)   % if over holdDur show Gauss 
                         o.hProbe{o.oriNum}.position = o.hProbe{o.targOri}.position;
                         o.targOri = o.oriNum;
                    end
                end
                %***** 
                if ( (currentTime - o.FixTime - o.FixJit) > (2*o.P.holdDur) )
                     drop = 1;  % give reward
        ...
        if (o.PFrameCount < o.MaxFrame)
            ...
            o.ProbeHistory(o.PFrameCount,5) = 0;  % assume not visible, unless drawn
            if (drop == 1)  % will be moving to state 2 or 3
                ...
                if (o.state < 3)
                   ...
                   if  (o.stimOn > 0)
                       if (currentTime <= (o.stimOn+o.P.stimDur))
                           o.ProbeHistory(o.PFrameCount,5) = 1; % make visible
                       end
                       if (o.FixTime ~= 0)
                           if (currentTime >= (o.FixTime + o.FixJit))
                               o.ProbeHistory(o.PFrameCount,5) = 1; % make visible
                           end
                       end
                   end
    ...
    function keepgoing = continue_run_trial(o,screenTime)
        ...
        if (o.PFrameCount)
           ...
           %********mark screen flip with stim onset
           if (o.stimOn == 0)
                  if (o.ProbeHistory(o.PFrameCount,3) > 0)
                      o.stimOn = screenTime;
                  end
           end
           %*************
    ...
    function drop = state_and_screen_update(o,currentTime,x,y) 
        ...
        %******changing some state info here:
        %****** state 1 is waiting for the subject to land on the target
        %****** state 2 is landed on target, holding to give reward
        %****** state 3 is reward, show Gauss
        %****** state 4 is now face reward
        ...
        if (drop)
            ...
            if (rn > (1-o.P.probFace))
                ...
                o.state = 3;  % move to a period with no probe target
            end
        end
        ...
        if (o.rewardTime > 0)
            if ( (currentTime - o.rewardTime) > o.rewardGap)
                o.rewardTime = 0;
                o.reset_probe_location_and_texture(0,x,y); % make it jump, firstime=1
                ...
                if (o.state == 4)  
                    if ( (currentTime - o.rewardTime) > o.faceTime)
                        o.state = 3;  % stop showing face, give extra juice
                        drop = 1;
                    end
                end
            end
        ...
        % Draw probe stimuli
        if (o.state < 3)
           if (o.ProbeHistory(o.PFrameCount,5) == 1)
